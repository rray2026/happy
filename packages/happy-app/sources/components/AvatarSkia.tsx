...
export const AvatarSkia = React.memo((props: AvatarProps) => {
    ...
    const rects = React.useMemo(() => {
        ...
        for (let row = 0; row < GRID_SIZE; row++) {
            for (let col = 0; col < GRID_SIZE; col++) {
                positions.push({ x: col * cellSize, y: row * cellSize, colorIndex: colorIndex++ });
            }
        }
        return positions;
    }, [cellSize]);
    ...
    return (
        <View style={{ width: size, height: size, overflow: 'hidden', borderRadius: square ? 0 : size / 2 }}>
            <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block' }}>
                {rects.map((r, i) => (
                    <rect key={i} x={r.x} y={r.y} width={cellSize} height={cellSize} fill={pixelColors[r.colorIndex]} />
                ))}
            </svg>
        </View>
    );
