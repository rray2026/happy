...
function printUsage(): void {
    console.log(
        [
            ...
            '',
            chalk.bold('Usage:'),
            '  cowork-agent serve [flags] [-- agent-args…]',
            '',
            chalk.bold('Flags:'),
            '  --claude             Use Claude Code CLI (default)',
            '  --gemini             Use Gemini CLI via ACP',
            '  --model, -m NAME     Pass --model NAME to the agent CLI',
            '  --workdir, -w PATH   Working directory for sessions (default: cwd)',
            '  --config, -c PATH    JSON config file (default: ~/.cowork-agent/config.json if present)',
            '',
            chalk.bold('Precedence:'),
            ...
            '  COWORK_AGENT_WORKDIR    Working directory for sessions (overrides cwd, beneath CLI flag)',
