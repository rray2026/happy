...
export function ChatScreen() {
    ...
    /** True while a programmatic scroll-to-bottom animation is in flight.
     *  Used to suppress handleScroll mid-animation: without this, scroll
     *  events fired mid-flight see scrollTop in the middle of the viewport,
     *  flip atBottomRef to false, and any items update that lands in the
    ...
    useEffect(() => {
        ...
        const t = setTimeout(() => { autoScrollingRef.current = false; }, 450);
    ...
    const handleScroll = useCallback(() => {
        ...
        if (!el) return;
        // Ignore scroll events generated by an in-flight programmatic scroll
        // — they'd otherwise flip atBottomRef to false mid-animation and
        // wedge the auto-follow off for any items batch arriving in the
        // same window.
    ...
    const voiceMode = useVoiceMode({
        ...
        ttsVoiceZh: settings.ttsVoiceZh || undefined,
        ttsRateZh: settings.ttsRateZh,
        ttsVoiceEn: settings.ttsVoiceEn || undefined,
        ttsRateEn: settings.ttsRateEn,
