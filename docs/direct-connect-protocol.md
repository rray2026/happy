...
`happy serve` 启动一个嵌入式 WebSocket 服务器，webapp 直接扫描 QR 码连接，无需经过云服务器中转。本文档描述 webapp 与 CLI 之间的全部通信协议。

相关源码：
- CLI：`packages/happy-cli/src/server/`（`types.ts` / `wsServer.ts` / `directAuth.ts` / `sessionStore.ts`）
- CLI：`packages/happy-cli/src/commands/serve.ts`
- Webapp：`packages/happy-app/sources/sync/directSocket.ts`
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `HAPPY_SERVE_PORT` | `4000` | 服务器监听端口 |
| `HAPPY_SERVE_ENDPOINT` | `ws://localhost:4000` | 写入 QR 的地址（webapp 实际连接此地址） |
